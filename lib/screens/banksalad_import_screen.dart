...
import '../services/banksalad_import_coordinator.dart';
...
class BanksaladImportScreen extends ConsumerStatefulWidget {
  const BanksaladImportScreen({super.key, this.debugPickFile});

  /// Test-only seam replacing the platform .xlsx file picker/read, for the
  /// same reason as SettingsScreen's `debugPickBackupSource`: file_selector's
  /// platform channel isn't available under `flutter test`. Returns the
  /// picked file's display name and raw bytes, or `null` if the (fake)
  /// picker was cancelled. Left `null` in production, so the real
  /// picker/read always runs.
  @visibleForTesting
  final Future<(String name, Uint8List bytes)?> Function()? debugPickFile;

  @override
  ConsumerState<BanksaladImportScreen> createState() =>
      _BanksaladImportScreenState();
}
...
class _BanksaladImportScreenState extends ConsumerState<BanksaladImportScreen> {
  ...
  Future<void> _pickFile() async {
    ...
    if (widget.debugPickFile != null) {
      final picked = await widget.debugPickFile!();
      // The fake picker can be gated by a test to stay pending across a
      // pop — same disposal risk as the real picker below.
      if (!mounted || picked == null) return;
      (pickedName, bytes) = picked;
    } else {
      const typeGroup = XTypeGroup(label: 'xlsx', extensions: ['xlsx']);
      final file = await openFile(acceptedTypeGroups: [typeGroup]);
      // The OS file picker can stay open long enough for the user to
      // navigate away (back button) before it resolves — this State may
      // already be disposed by the time we get here.
    ...
    try {
      ...
      final ledgerResult = TransactionImportService.parseBanksaladLedger(
        ledgerRows,
      );
      ...
      final validTx = allParsed
          .where((r) => r.isValid)
          .map((r) => r.transaction!)
          .toList();
      final existing = ref.read(transactionsProvider);
      final newTx = TransactionImportService.filterDuplicates(
        validTx,
        existing,
      );
  ...
  Future<void> _confirmImport() async {
    ...
    try {
      // The coordinator serializes this against any other in-flight import
      // and, on failure, restores storage to exactly its pre-import state
      // (transactions and same-day snapshots included) before rethrowing —
      // see BanksaladImportCoordinator's doc comment.
      await ref
          .read(banksaladImportCoordinatorProvider)
          .importCombined(transactions: newTx, snapshot: snapshot);
  ...
  Widget build(BuildContext context) {
    ...
    final snapshot = _assetSnapshot;
    final canConfirm =
        !_isImporting && ((newTx?.isNotEmpty ?? false) || snapshot != null);
      ...
      body: PageContentBounds(
        ...
        child: AbsorbPointer(
          ...
          child: ListView(
            ...
            children: [
              const Text(
                '뱅크샐러드에서 내려받은 .xlsx 파일을 선택하세요. 거래내역과 자산현황을 한 번에 가져와요.',
              ),
              ...
              if (newTx != null) ...[
                ...
                ...newTx
                    .take(20)
                    .map((tx) => TransactionTile(transaction: tx, dense: true)),
                if (newTx.length > 20)
                  Padding(
