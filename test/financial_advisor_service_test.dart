...
void main() {
  ...
  group('LocalRuleFinancialAdviceSource', () {
    ...
    test('flags a category increase of 30% or more', () async {
      ...
      expect(advice.any((a) => a.category == 'spending' && a.message.contains('식비')), isTrue);
    });
    ...
    test('praises a category decrease of 30% or more', () async {
      ...
      final advice = await source.generateAdvice(context);
      expect(advice.any((a) => a.category == 'spending' && a.message.contains('줄였어요')), isTrue);
    });
    ...
    test('flags a goal that is behind its expected pace', () async {
      const context = FinancialAdviceContext(
        ...
        goalProgress: [GoalProgressSummary(title: '여행자금', actualProgress: 0.2, expectedProgress: 0.5)],
      );
      final advice = await source.generateAdvice(context);
      expect(
        advice.any((a) => a.category == 'goal' && a.message.contains('여행자금') && a.message.contains('느려요')),
        isTrue,
      );
    });
    ...
    test('praises a goal that is on or ahead of pace', () async {
      const context = FinancialAdviceContext(
        currentMonthExpenseByCategory: {},
        previousMonthExpenseByCategory: {},
        goalProgress: [GoalProgressSummary(title: '여행자금', actualProgress: 0.6, expectedProgress: 0.5)],
      );
      final advice = await source.generateAdvice(context);
      expect(advice.any((a) => a.category == 'goal' && a.message.contains('잘 따라가고')), isTrue);
    });
    ...
    test('comments on net worth direction when available', () async {
      ...
      final advice = await source.generateAdvice(context);
      expect(advice.any((a) => a.category == 'networth' && a.message.contains('늘었어요')), isTrue);
    });

    test('falls back to a general encouragement message when nothing else applies', () async {
      const context = FinancialAdviceContext(
        currentMonthExpenseByCategory: {},
        previousMonthExpenseByCategory: {},
        goalProgress: [],
  ...
  group('FinancialAdvisorService.shouldRefresh', () {
    ...
    test('is false right after a refresh', () {
      expect(service.shouldRefresh(UserProfile(lastAdviceRefresh: DateTime.now())), isFalse);
    });

    test('is true once the refresh interval has elapsed', () {
      final stale = DateTime.now().subtract(const Duration(hours: 25));
      expect(service.shouldRefresh(UserProfile(lastAdviceRefresh: stale)), isTrue);
    });
  });
  ...
  group('FinancialAdvisorService.buildContext', () {
    ...
    setUp(() async {
      tempDir = await Directory.systemTemp.createTemp('human_status_advisor_test_');
      ...
      if (!Hive.isAdapterRegistered(5)) Hive.registerAdapter(AssetSnapshotAdapter());
      ...
      storage.assetSnapshotsBox = await Hive.openBox<AssetSnapshot>('snap_$suffix');
    ...
    Transaction tx(double amount, DateTime date, {String category = '식비'}) => Transaction(
          id: const Uuid().v4(),
    ...
    test('computes expectedProgress from elapsed time for a dated financial goal', () async {
      final now = DateTime.now();
      final goal = Goal(
        id: 'g1',
        title: '여행자금',
    ...
    test('computes netWorthChangePercent from the two most recent snapshots', () async {
      final now = DateTime.now();
      await storage.saveAssetSnapshot(AssetSnapshot(
        id: 's1',
