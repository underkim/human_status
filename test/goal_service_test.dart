...
Quest _quest({required String goalId, QuestStatus status = QuestStatus.active}) {
  return Quest(
    id: const Uuid().v4(),
    title: 'q',
    description: '',
...
void main() {
  group('LocalRuleGoalDecompositionSource', () {
    ...
    test('generates quests linked to the goal, matching its statId', () async {
      final goal = _goal();
      final quests = await source.decompose(goal: goal, stats: [], existingQuests: [], count: 3);
    ...
    test('always includes a kick-off quest referencing the goal title', () async {
      final goal = _goal();
      final quests = await source.decompose(goal: goal, stats: [], existingQuests: [], count: 1);
  ...
  group('GoalService.progress', () {
    ...
    test('financial goal progress is currentAmount/targetAmount, clamped to 1', () {
      expect(service.progress(_goal(targetAmount: 100, currentAmount: 50), []), 0.5);
      expect(service.progress(_goal(targetAmount: 100, currentAmount: 200), []), 1.0);
    });

    test('financial goal with a zero target guards against division by zero', () {
      expect(service.progress(_goal(targetAmount: 0, currentAmount: 0), []), 0.0);
    });

    test('non-financial goal progress is the linked-quest completion ratio', () {
      final goal = _goal();
      final quests = [
  ...
  group('GoalService.isAutoCompletable', () {
    ...
    test('true when all linked quests are resolved and at least one completed', () {
      final goal = _goal();
      final quests = [
        _quest(goalId: goal.id, status: QuestStatus.completed),
